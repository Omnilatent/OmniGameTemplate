...
public static class BuildCommand
{
    ...
    private const string GRADLE_VERSION = "GRADLE_VERSION";
    private const string ANDROID_CREATE_SYMBOL = "ANDROID_CREATE_SYMBOL";
    ...
    [MenuItem(itemName: "Tools/Custom/Build using Script")]
    public static void PerformBuildInEditor()
    {
        ...
        var buildPath = $"{Application.dataPath}/../Builds/";
        ...
        Console.WriteLine($":: Done with build. Build path: {fixedBuildPath}");
        if (buildTarget == BuildTarget.Android) { HandleShrinkSymbols(fixedBuildPath); }
    ...
    {
        if (!TryGetEnv(GRADLE_VERSION, out string gradleVersion))
        {
            return;
        }

        EditorPrefs.SetBool("GradleUseEmbedded", false);
        string gradlePath = $"./gradle-{gradleVersion}";

        EditorPrefs.SetString("GradlePath", gradlePath);
        Console.WriteLine(":: PreProcessBuild - changed path: " + EditorPrefs.GetString("GradlePath"));
    }

    public static void HandleCreateSymbols()
    {
        bool createSymbol = EditorUserBuildSettings.buildAppBundle;
        if (TryGetEnv(ANDROID_CREATE_SYMBOL, out string value) && bool.TryParse(value, out bool forceCreateSymbol))
        {
            Console.WriteLine($":: Force create Android symbols: {forceCreateSymbol}");
            createSymbol = forceCreateSymbol;
        }

        EditorUserBuildSettings.androidCreateSymbolsZip = createSymbol;
    }

    public static void SetArchitecture()
    {
        PlayerSettings.Android.targetArchitectures = AndroidArchitecture.ARM64 | AndroidArchitecture.ARMv7;
    }

    static void HandleShrinkSymbols(string fixedBuildPath)
    {
# if UNITY_2020
        if (EditorUserBuildSettings.androidCreateSymbolsZip)
        {
            var searchs = Directory.GetFiles(Path.GetDirectoryName(fixedBuildPath), "*.symbols.zip");
            if (searchs.Length < 1) { Console.WriteLine($":: No symbols.zip found at {fixedBuildPath}, skipping shrinking symbols."); }
            else
            {
                string symbolPath = searchs[0];
                Debug.Log($":: Shrinking symbol at {symbolPath}.");
                Exception exception = null;
                try { AndroidSymbolShrinker.ShrinkSymbols(symbolPath); }
                catch (Exception e)
                {
                    exception = e;
                ...
                if (exception == null)
                {
                    Console.WriteLine($":: Shrinked success, deleting symbols.zip.");
